...
class Synth:
    ...
    def play_success_sound(self):
        self.fs.program_select(
            0, self.sfid, 0, 8
        )  # channel 0, bank 0, program 8 (Celesta)
        notes = [72, 76, 79]
    ...
    def play_measure(
        self, measure_index, midi_teacher, set_index_callback=None, reset_to_index=None
    ):
        chords, times, _, _, (start_index, _) = midi_teacher.get_notes_for_measure(
            measure_index
        )
        self.play_notes(
            chords,
            times,
            start_index=start_index if set_index_callback else None,
            set_index_callback=set_index_callback,
            reset_to_index=reset_to_index,
        )
    ...
    def play_notes(
        ...
        times,
        start_index=None,
        set_index_callback=None,
        reset_to_index=None,
    ):  # TODO: playback using actual midi events
        ...
        if start_index is not None and set_index_callback is None:
            raise ValueError(
                "Error: set_index_callback must be provided if start_index is provided"
            )
        if reset_to_index is not None and set_index_callback is None:
            raise ValueError(
                "Error: set_index_callback must be provided if reset_to_index is provided"
            )
