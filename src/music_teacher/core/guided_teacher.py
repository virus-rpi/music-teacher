...
class PlaybackMeasureTask(Task):
    ...
    def on_start(self):
        _, _, self.notes_x, _, _ = self.teacher.midi_teacher.get_notes_for_measure(
            self.measure
        )
        self.teacher.current_section_visual_info = (
            [self.notes_x[0], self.notes_x[-1]] if self.notes_x else None
        )
    ...
    def on_tick(self, pressed_notes, pressed_note_events, _):
        if (
            self.not_played_for > 5 and not self.played
        ):  # give it a bit of time to render highlight
            self.teacher.synth.play_measure(
                self.measure,
                self.teacher.midi_teacher,
...
class PracticeSectionTask(Task):
        ...
        self.section_index = next(
            (
                ...
                for i, sec in enumerate(
                    self.teacher.split_measure_into_sections(self.measure)
                )
                if sec.start_idx == self.section.start_idx
                and sec.end_idx == self.section.end_idx
            ),
            None,
        )
    ...
    def _handle_midi_events(
        self, pressed_notes: set[int], midi_events: list[mido.Message]
    ):
        if len(self.recording) == 0 and len(midi_events) > 0:
            ...
            times = [
                ...
                if hasattr(ev, "time")
                and hasattr(ev, "note")
                and getattr(ev, "note", None) in first_chord
            ]
            if not times:
                return
            earliest = min(times)
        ...
        if self.timer_start is not None:
            for msg in midi_events:
                ...
                delta_ms = (
                    max(int((t - self._last_record_time) * 1000), 0)
    ...
    def _handle_evaluate(self):
        ...
        evaluator = Evaluator(
            self.recording,
            self.teacher.midi_teacher.query_notes_and_pedals(
    ...
    def _save_pass(self, evaluator):
        if self.section_index is not None:
            self.teacher.save_pass(
                str(self.measure),
                str(self.section_index),
                self.teacher.pass_index.get(str(self.measure), {}).get(
                    str(self.section_index), 0
                ),
            ...
            print(
                f"Could not find section {self.section.start_idx} {self.section.end_idx} in measure {self.measure}"
            )


class PracticeMeasureTask(PracticeSectionTask):
    def __init__(self, teacher: "GuidedTeacher", measure):
        chords, times, xs, _, (measure_start_index, _) = (
            teacher.midi_teacher.get_notes_for_measure(measure)
        )
...
class PracticeTransitionTask(PracticeSectionTask):
    def __init__(self, teacher, from_measure, to_measure):
        from_chords, from_times, from_xs, _, (from_start_idx, _) = (
            teacher.midi_teacher.get_notes_for_measure(from_measure)
        )
        to_chords, to_times, to_xs, _, (to_start_idx, _) = (
            teacher.midi_teacher.get_notes_for_measure(to_measure)
        )
        ...
        section = MeasureSection(
            section_chords, section_times, section_xs, start_idx, end_idx
        )
...
class GuidedTeacher:
    def __init__(
        self, midi_teacher: MidiTeacher, synth: Synth, save_system: SaveSystem = None
    ):
        ...
        self._save_root = os.path.join(
            save_system.save_root, save_system.guided_teacher_data_dir
        )
    ...
    def start(self):
        self.is_active = True
        self._loop_info_before = (
            self.midi_teacher.loop_enabled,
            self.midi_teacher.loop_start,
            self.midi_teacher.loop_end,
        )
        self.load_state()
    ...
    def generate_tasks_for_measure(self, measure_index):
        ...
        if measure_index + 1 < len(self.midi_teacher.measures):
            self.tasks.append(
                PracticeTransitionTask(self, measure_index, measure_index + 1)
            )
    ...
    def next_task(self):
        if self.current_task:
            ...
            if not isinstance(
                self.current_task, (GenerateNextMeasureTasks, PlaybackMeasureTask)
            ):
    ...
    def split_measure_into_sections(self, measure_index):
        measure_chords, measure_times, measure_xs, _, (measure_start_index, _) = (
            self.midi_teacher.get_notes_for_measure(measure_index)
        )
        ...
        def add_section(section_start, section_end):
            ...
            sections.append(
                MeasureSection(
                    section_chords, section_times, section_xs, start_index, end_index
                )
            )
    ...
    def save_pass(
        ...
        recording: mido.MidiTrack,
        section: MeasureSection,
    ):
        with self.save_system as s:
            if pass_idx == 0:
                s.save_file(
                    f"measure_{measure_idx}/section_{section_idx}/section.json",
                    json.dumps(
                        {"section": asdict(section), "timestamp": time.time()},
                        indent=2,
                        default=str,
                    ),
                )
            s.save_file(
                f"measure_{measure_idx}/section_{section_idx}/pass_{pass_idx}.json",
                json.dumps(
                    {"evaluation": asdict(evaluation), "timestamp": time.time()},
                    indent=2,
                    default=str,
                ),
            )
            if recording is not None and isinstance(recording, mido.MidiTrack):
                ...
                mid.save(
                    s.get_absolute_path(
                        f"measure_{measure_idx}/section_{section_idx}/pass_{pass_idx}.mid"
                    )
                )
        self.pass_index[measure_idx] = self.pass_index.get(measure_idx, {})
        self.pass_index[measure_idx][section_idx] = (
            self.pass_index[measure_idx].get(section_idx, 0) + 1
        )
